...
export function BottomNavBar() {
  ...
  const tabClass = (active: boolean) =>
    [
      'flex flex-col items-center justify-center px-3 py-1 active:scale-90 transition-transform w-[72px] rounded-xl',
      active
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 w-full flex justify-around items-center px-2 pb-safe h-16 bg-surface-container-lowest z-[500] border-t border-outline-variant shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
      ...
      <button onClick={goStations} className={tabClass(isStations)}>
        ...
        <span className="text-[10px] font-medium mt-1">Stations</span>
      </button>
      ...
      <button onClick={goTrends} className={tabClass(isTrends)}>
        ...
        <span className="text-[10px] font-medium mt-1">Tendances</span>
      </button>
